...
public class SeagullEntity extends AnimalEntity implements SmartBrainOwner<SeagullEntity> {
    ...
    public BrainActivityGroup<? extends SeagullEntity> getCoreTasks() {
        return BrainActivityGroup.coreTasks(
                ...
                new SetWalkTargetToAttackTarget<>().whenStarting((mobEntity) -> {
                    setFlying();
                }),
    ...
    public BrainActivityGroup<? extends SeagullEntity> getIdleTasks() {
        return BrainActivityGroup.idleTasks(
                new FirstApplicableBehaviour<SeagullEntity>(
                        new SetRetaliateTarget<>().alertAlliesWhen((owner, attacker) ->
                                true).isAllyIf((owner, ally) -> owner.getClass().isAssignableFrom(ally.getClass()) && BrainUtils.getTargetOfEntity(ally) == null && (!(owner instanceof TameableEntity pet) || pet.getOwner() == ((TameableEntity) ally).getOwner()) &&
                                !ally.isTeammate(BrainUtils.getMemory(owner, MemoryModuleType.HURT_BY_ENTITY)))
                ),
                new OneRandomBehaviour<>(
                        new SetRandomSeagullFlightTarget<>().setRadius(10).whenStarting((pathAwareEntity) -> {
                            setFlying();
                        }),
                        new SetRandomSeagullWalkTarget<>().setRadius(5, 3).dontAvoidWater().startCondition(Entity::isOnGround).whenStarting((pathAwareEntity) -> {
                            setGrounded();
                        }),
